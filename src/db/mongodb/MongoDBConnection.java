...
//this static import is for static eq function 
...
public class MongoDBConnection implements DBConnection {
	...
	public boolean registerUser(String userId, String password, String firstname, String lastname) {
		...
		try {
//			if (db.getCollection("users").find(eq("user_id")) == null) {
...
//			}
	...
	public void saveItem(Item item) {
		...
		//itemid and item are unique record. so only check the first item. check whether itemid exist in DB before or not
		if (iterable.first() == null) {
			db.getCollection("items")
					.insertOne(new Document().append("item_id", item.getItemId()).append("distance", item.getDistance())
							.append("name", item.getName()).append("address", item.getAddress())
